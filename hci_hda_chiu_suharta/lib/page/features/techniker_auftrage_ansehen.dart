      ...
      return Scaffold(
        ...
        body: Column(
          children: [
            ...
            const Text('Available Bookings', style: TextStyle(fontSize: 26)),
            Flexible(
              child: StreamBuilder<QuerySnapshot>(
                stream:
                    FirebaseFirestore.instance.collection('booking').snapshots(),
                builder: (BuildContext context,
                  ...
                  var availableBookings = snapshot.data!.docs.where((booking){
                    final data = booking.data() as Map<String, dynamic>;
                    return !data.containsKey('chosen') || !(data['chosen'] as bool? ?? false);
                  }).toList();

                  if(availableBookings.isEmpty){
                    return const Text('No Available Booking');
                  }
                  return ListView.builder(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            ...
                            Text('Price: ${data['price']}',
                                style: bodyStyle),
                            Text(
                                'Component: ${data['komponente'].join(', ')}',
                                style: bodyStyle),
